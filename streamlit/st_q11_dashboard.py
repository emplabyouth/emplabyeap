...
def create_unified_header():
    ...
    if img_base64:
        ...
        st.subheader("Advocacy & Partnerships Dashboard")

    # Add page-specific title and subtitle with blue background
    st.subheader("🤝 Advocacy & Partnerships")
    st.markdown("""This tab highlights the ILO's work in **advocacy and partnerships** to promote decent work for youth at global, regional, and national levels.

Activities reported under this cluster include engaging in **policy dialogue**, participating in **global and regional forums**, and collaborating with a wide range of **partners**, including UN agencies, civil society organizations, and the private sector.

Outputs also cover the organization of **events, campaigns, and joint initiatives** to raise awareness, mobilize action, and build alliances for youth employment, such as the [Global Initiative on Decent Jobs for Youth](https://www.decentjobsforyouth.org/).

**Additional resources:**
- [Decent Jobs for Youth | The Global Initiative](https://www.decentjobsforyouth.org/)
- [Partnerships for youth employment | International Labour Organization](https://www.ilo.org/employment/areas/youth-employment/partnerships/lang--en/index.htm)
- [UN Youth Strategy | United Nations](https://www.un.org/youthenvoy/youth-un/)""")
    st.markdown("---")

class Q11DataProcessor:
    """Data processor specifically for Q11 (Advocacy & Partnerships)"""
    ...
    def load_data(self):
        ...
        # Load Q11 data
        q11_file = os.path.join(project_root, 'orignaldata', 'PART3_base_dataQ11.csv')
        
        if os.path.exists(q11_file):
            self.original_data = safe_read_csv(q11_file)
            self.data = self.original_data.copy()
        else:
            st.error(f"Q11 data file not found: {q11_file}")
    ...
    def get_works_count_data(self):
        """Get works count data for Q11 - returns dict format compatible with original"""
        if self.data.empty:
            return {}
        
        # Calculate statistics for Q11
...
def create_theme_detail_list(data_processor):
    ...
    preferred_columns = [
        ...
        'Q11_1_Advocacy_partnerships_type', 'Q11_2_Advocacy_partnerships_title',
        'Q11_3_Advocacy_partnerships_description', 'Q11_4_Advocacy_partnerships_partners'
    ]
...
def create_layout():
    """Create the main Streamlit layout for Q11"""
    ...
    if STYLES_AVAILABLE:
        ...
        st.set_page_config(
            page_title="ILO YEAP - Advocacy & Partnerships",
            page_icon="🤝",
            layout="wide",
    ...
    data_processor = Q11DataProcessor()
    ...
    if data_processor.data.empty:
        st.warning("No data available for Advocacy & Partnerships with current filters.")
        return
    ...
    if works_count_data:
        count_fig = create_theme_count_chart(works_count_data, current_theme="Q11")
        if count_fig:
    ...
    # Define Q11-specific charts based on original themes configuration
    q11_charts = [
        ('Funding source (Options: regular budget or extrabudgetary)', 'pie', 'Funding Source Of Advocacy & Partnerships Outputs'),
        ('Focus (Options: Youth only or Youth is one of the target groups)', 'pie', 'Target Group Of Advocacy & Partnerships Outputs'),
        ('Type of advocacy/partnership (Options: Event, or Campaign, or Joint initiative)', 'bar', 'Types Of Advocacy & Partnerships Delivered')
    ]
    
    has_frequency_data = False
    for field_name, chart_type, chart_title in q11_charts:
    ...
    if detail_df is not None and not detail_df.empty:
        ...
        page_key = 'q11_current_page'
        if page_key not in st.session_state:
            st.session_state[page_key] = 1
        
        # Get current page data
        items_per_page = st.session_state.get('q11_items_per_page', 50)
        ...
        with control_col1:
            ...
            new_items_per_page = st.selectbox(
                ...
                key="q11_items_per_page_select"
            )
            
            if new_items_per_page != items_per_page:
                st.session_state.q11_items_per_page = new_items_per_page
                st.session_state[page_key] = 1
                st.rerun()
