...
def create_unified_header():
    ...
    if img_base64:
        ...
        st.subheader("Knowledge Development & Dissemination Dashboard")

    # Add page-specific title and subtitle with blue background (matching original file)
    st.subheader("📚 Knowledge Development & Dissemination")
    st.markdown("""This tab highlights the ILO's efforts to generate, analyse and share **evidence-based knowledge** to guide the design and implementation of **youth employment strategies**.
...
class Q6DataProcessor:
    """Data processor specifically for Q6 (Knowledge Development & Dissemination)"""
    ...
    def load_data(self):
        ...
        # Load Q6 data
        q6_file = os.path.join(project_root, 'orignaldata', 'PART3_base_dataQ6.csv')
        
        if os.path.exists(q6_file):
            self.original_data = safe_read_csv(q6_file)
            self.data = self.original_data.copy()
        else:
            st.error(f"Q6 data file not found: {q6_file}")
    ...
    def get_works_count_data(self):
        """Get works count data for Q6 - returns dict format compatible with original"""
        if self.data.empty:
            return {}
        
        # Calculate statistics for Q6
...
def create_theme_detail_list(data_processor):
    ...
    preferred_columns = [
        ...
        'Q6_1_Knowledge_products_type', 'Q6_2_Knowledge_products_title',
        'Q6_3_Knowledge_products_description', 'Q6_4_Knowledge_products_target_audience',
        'Q6_5_Knowledge_products_dissemination_channels'
    ]
...
def create_layout():
    """Create the main Streamlit layout for Q6"""
    ...
    if STYLES_AVAILABLE:
        ...
        st.set_page_config(
            page_title="ILO YEAP - Knowledge Development & Dissemination",
            page_icon="📚",
            layout="wide",
...
- [Statistics on youth - ILOSTAT](https://ilostat.ilo.org/topics/youth/)""")
    ...
    data_processor = Q6DataProcessor()
    ...
    if data_processor.data.empty:
        st.warning("No data available for Knowledge Development & Dissemination with current filters.")
        return
    ...
    if works_count_data:
        count_fig = create_theme_count_chart(works_count_data, current_theme="Q6")
        if count_fig:
    ...
    # Define Q6-specific charts based on original themes configuration
    q6_charts = [
    ...
    for field_name, chart_type, chart_title in q6_charts:
    ...
    if detail_df is not None and not detail_df.empty:
        ...
        page_key = 'q6_current_page'
        if page_key not in st.session_state:
            st.session_state[page_key] = 1
        
        # Get current page data
        items_per_page = st.session_state.get('q6_items_per_page', 50)
        ...
        with control_col1:
            ...
            new_items_per_page = st.selectbox(
                ...
                key="q6_items_per_page_select"
            )
            
            if new_items_per_page != items_per_page:
                st.session_state.q6_items_per_page = new_items_per_page
                st.session_state[page_key] = 1
                st.rerun()
